...
​	CAN模块提供3个DMA触发源输出，每个接口寄存器具有1个。在CAN_CTL寄存器中的DE1,DE2,DE3位进行对应的使能。

​	IF1和IF2寄存器的更新分别由对IF1和IF2命令寄存器的写访问发起。一旦启用，在IF1CMD或IF2CMD寄存器中设置DMAactive位将导致下一次相应接口可用时发出DMA请求。

​	IF3寄存器的内容可以在接收到CAN消息时自动更新，这些消息对象被编程为自动IF3更新。也就是说，当启用IF3 DMA请求时，所有IF3更新都会触发DMA请求。

​	当DCAN内部IFx更新完成时，DMA请求被激活并保持活动状态，直到第一次访问相关的IFx寄存器之一；也就是说，在对IF寄存器集进行第一次读或写访问之后，将清除DMA请求
...
​	如果CAN核心的移位寄存器准备加载并且接口寄存器和消息RAM之间没有数据传输，消息有效寄存器的MsgVal位和发送请求寄存器的TxRqst位被评估。挂起传输请求的最高优先级的有效消息对象被消息处理器装载到移位寄存器中并且开始发送。消息对象的NewDat被复位。
...
​	如果使用的消息对象数量不满足被用来当作永久消息对象，发送对象可以被动态管理。CPU可以写整个消息（包括ID，DLC，DATA）到接口寄存器。**在传输整个消息对象的内容到消息对象之前，IF命令寄存器的[23:16]位域应被设置位0xB7**。在这个操作之前MsgVal 和 TxRqst都不需要复位。
