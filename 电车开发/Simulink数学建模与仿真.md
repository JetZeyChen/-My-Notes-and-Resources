...
​		通过输入两个阶跃信号给到左右电机输入模块，作为电机的扭矩输出，阶跃时间为1s，给一定时间的延时，可以有利于观察前后的变化，然后阶跃值给1。运行仿真按钮，在输出端添加查看器Scope，仿真完成后可以看到相关的仿真信息。
...
​	P1: 对于具有反馈的加法器模块，如果输出作为反馈部分，则一般需要在反馈输入加入一个单元延迟单元，其在离散模块库部分，Unit Delay 顾名思义，其会延迟一个计算步长后再获取值，在此之前初始值都由人为设定，默认为0。
